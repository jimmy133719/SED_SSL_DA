...
if __name__ == '__main__':
    ...
    # Use Meanteacher
    parser.add_argument("-nmt", '--no_meanteacher', action="store_true",
    ...
    other_description = '_fpn_0' # name your own model
    if f_args.no_meanteacher:
        other_description += '_nomeanteacher'
    ...
    if start_epoch == 0:
        ...
        # torch.save(crnn.state_dict(), 'crnn_init')
        pdb.set_trace()
    # resume training
    else:
    ...
    for epoch in range(start_epoch, cfg.n_epoch):
        ...
        if not f_args.no_meanteacher:
            ...
            loss_value = train(training_loader, crnn, optim, epoch,
                            mask_weak=weak_mask, mask_strong=strong_mask, adjust_lr=cfg.adjust_lr)
        ...
        # Evaluate weak
        weak_metric = get_f_measure_by_class(crnn, len(cfg.classes), validation_dataloader_weak)
